...
from tests.schema.accounts import (
    ...
    CreateAccountResponseTestSchema,
    UpdateAccountBalanceRequestTestSchema,
    UpdateAccountBalanceResponseTestSchema,
    CreateAccountRequestTestSchema
)
...
@accounts_mock_router.post('', response_model=CreateAccountResponseTestSchema)
async def create_account_view(
        request: CreateAccountRequestTestSchema,
        scenario: Scenario = Depends(get_scenario_http)
):
    return await loader.load_http(
        file=f"create_account/{scenario}.json",
        model=CreateAccountResponseTestSchema
    )


@accounts_mock_router.post(
    '/update-account-balance',
    response_model=UpdateAccountBalanceResponseTestSchema
)
async def update_account_balance_view(
        request: UpdateAccountBalanceRequestTestSchema,
        scenario: Scenario = Depends(get_scenario_http)
):
    return await loader.load_http(
        file=f"update_account_balance/{scenario}.json",
        model=UpdateAccountBalanceResponseTestSchema
    )
